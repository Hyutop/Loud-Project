...
module.exports = async function buildProject(config) {
  try {
    ...
    console.log("[+] Running WebPack for new.js in buildSrcDir...");
    ...
    if (buildType === "pkg") {
      console.log("[+] Obfuscating files for pkg build...");

      const ObfuscateFilePKG = require("./cryptPreset/PkgObfuscation");
      let params = {
        filePath: path.join(buildSrcDir, "index.js"),
        outputPath: path.join(buildSrcDir, "index.js"),
      };
      await ObfuscateFilePKG(params);
      ...
      console.log("[+] Obfuscating files for bun build...");
      const ObfuscateFileBun = require("./cryptPreset/BunObfuscation");
      let params = {
        filePath: path.join(buildSrcDir, "index.js"),
        outputPath: path.join(buildSrcDir, "index.js"),
      };
      await ObfuscateFileBun(params);
      console.log("[+] Running bun build...");
      execSync("npm run bunBuild", { stdio: ["ignore", "pipe", "pipe"], cwd: buildSrcDir });
    }
...
async function runPkgBuild(fudPkgScript, fudDir) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn("node", [fudPkgScript], { cwd: fudDir, stdio: ["ignore", "pipe", "pipe"] });
...
async function CleanDirs(buildDir) {
  try {
